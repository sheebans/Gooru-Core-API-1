...
public interface ConstantProperties extends ErrorCodeProperties {
	...
	static final String RESOURCE_INCLUDE_FIELDS[] = { "*.resource", "*.assetURI", "*.brokenStatus", "*.category", "*.createdOn", "*.usernameDisplay", "*.profileImageUrl", "*.lastName", "*.firstName", "*.username", "*.description", "*.distinguish", "*.folder", "*.gooruOid", "*.resourceType",
			...
			"*.course", "*.customFieldValues.*", "*.publisher", "*.aggregator", "*.totalPages", "*.host", "*.resourceTags.*", "*.typeName", "*.s3UploadFlag", "*.sourceReference", "*.skills", "*.attributes.*", "*.feedback.*", "*.misc.*", "*.hlType", "*.answerId", "*.answerText", "*.answerType", "*.isCorrect", "*.sequence", "*.hintId", "*.hintText" };
